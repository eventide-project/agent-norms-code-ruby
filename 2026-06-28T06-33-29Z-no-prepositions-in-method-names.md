...
**Scope — method names only.** This governs **method/initializer names**, not prose or terminology. Prose deliberately *does* use prepositional phrasing where it carries meaning — e.g. inputs are described as **"sent to"** a method (the sent-to phrasing rule, in `vocabulary`). That is a description of a relationship, not an identifier; the preposition is load-bearing there and absent here. (The relation verb is the bare **"mediates"** — see the no-slang rule in `vocabulary` — so it is not an example of load-bearing prepositional prose.)

**How to apply:** When naming a method, name the action and stop. If you feel the pull to append `_from`/`_to`/`_in`/`_with`/`_into`/`_for`, that is the mean-bias tell — the argument the preposition would point at is already the parameter. Related: the name-literally-not-by-analogy rule and the general `solubility` term (both in `vocabulary`).
